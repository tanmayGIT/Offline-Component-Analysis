...
% for seeing the annotated segmented word on the full image

clear all;
close all;
clc;

load('segmentedDocImageRefImg.mat');
load('imagePathRefImg.mat');

for i = 1:1:size(segmentedDocImageRefImg,1)
   
    [~,~,imageComponent] = findApplicableForCell(segmentedDocImageRefImg{i,1}{1,3});
    imageFilePathRef = imagePathRefImg{i,1};
