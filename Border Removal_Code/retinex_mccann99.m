...
OP = double(Maximum)*double(ones([nrows ncols]));                    % initialize Old Product
for layer = 0:nLayers
...
IP = double(OPE(2+dif_row:(end-1+dif_row), 2+dif_col:(end-1+dif_col))) + ...
     double(RRE(2:(end-1),2:(end-1)) - RRE(2+dif_row:(end-1+dif_row), 2+dif_col:(end-1+dif_col)));
...
% Giving the padded area the values at the nearest particular boundary
...
NP = (double(OPE(2:(end-1),2:(end-1))) + double(IP))/2;    % Doing the averaging operation, the obtained
% value and the original image
