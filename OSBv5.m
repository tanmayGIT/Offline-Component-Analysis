...
function [pathcost,d,indxrow,indxcol,indexStorer] = OSBv5(t1,t2,warpwin,queryskip,targetskip,jumpcost)
% t1 = refWord
% t2 = testWord
[m,nFeature1] = size(t1);
[n,nFeature2] = size(t2);
...
if(nFeature1 == nFeature2) % each set containing same no. of feature
    Dist = zeros(m,n); % Initializing the array
   
    for i=1:1:m % xSize
        for j=1:1:n %ySize
            total = 0;
            for goFeature=1:nFeature1 %or nFeature2 as nFeature1 & nFeature2 are same
                total = total + (((t1(i,goFeature)-t2(j,goFeature))^2));
            end
            Dist(i,j) = sqrt(total);
        end
    end
...
[pathcost,indxrow,indxcol,indexStorer] = findpathDAG(matxE,warpwin,queryskip,targetskip,jumpcost);
...
%--------------------------------------------------------------------------
function [pathcost,indxrow,indxcol,indexStorer] = findpathDAG(matx,warpwin,queryskip,targetskip,jumpcost)
...
indexStorer(1,1) = indxrow;
indexStorer(1,2) = indxcol;
