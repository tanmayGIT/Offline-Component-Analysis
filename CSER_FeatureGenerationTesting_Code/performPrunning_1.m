function  [l1,l4,alfaU,alfaV] = performPrunning_1(testImg)
...
for rot = -rotationAngle:1:rotationAngle
    ...
    [S0Y_U] = getHistogram(rotatedImg);
    %     [rotRw rotCol] = size(rotatedImg);
    for i = l1:1:(ceil(l2+((l3-l2)/2)))
        %         indexes1 = find(rotatedImg(i-1,:));
        %         if((S0Y_U(i-1,1)) > 0)
        % if(S0Y(t1-1,1)>0)
        %             indexes2 = find(rotatedImg(i,:));
        if((i>1) && (i<  (size(S0Y_U,1))  ))
            calDiff = sqrt((S0Y_U(i-1,1)-S0Y_U(i,1))^2);
    ...
    for j = (ceil(l2+((l3-l2)/2))):1:l4
        %         indexes1 = find(S0Y_V(i-1,:));
        %         if((length(indexes1)) > 0)
        % if(S0Y(t1-1,1)>0)
        %             indexes2 = find(S0Y_V(i,:));
        if((j>1)&& (j<  (size(S0Y_U,1))  ))
            calDiff = sqrt((S0Y_U(j-1,1)-S0Y_U(j,1))^2);
