function [croppedImg] = TestwordSpottingBasicOperation(Img)
...
verticalHistMat = zeros(nRw,nCol);
conditionFlagOfX0 = 0;
...
for j = 1:1:nCol % iterate for each column
    ...
    if((length(indexes)) > 1)
        ...
        verticalHistMat((nRw-(nForeGrndPixels-1)):nRw,j) = 1;
...
if(conditionFlagOfX0 ~= 0)
    for j = 1:1:(ceil(nCol/3)) % iterate for each column
        tempMat = verticalHistMat((nRw-(L_2-1)):nRw,j:(j+(nColOfKernel-1)));
        %         if(j>18)
        %             disp('just check');
        %         end
...
if(conditionFlagOfX1 ~= 0)
    for j = X1:1:(ceil(nCol/3)) % iterate for each column
        tempMat = verticalHistMat((nRw-(L_2-1)):nRw,j:(j+nColOfKernel));
...
% %% for calculating the vertical limit
% B_2 = ceil((nCol*1)/30);
% % B_3 = ceil((nCol*1)/20);
% blackPixelMat = zeros(nRw,1);
% horiHistMat = zeros(nRw,nCol);
% for p = 1:(size(Img1,1))
%     indexes = find(Img1(p,:));
...
% % topStart = 1;
% % bottomEnd = nRw;
% for findTop = 1:1:(nRw/5)
%     tempMat = horiHistMat(findTop:((findTop+nRowOfKernel)-1),1:B_2); % generating the kernel
%
%     % checking if all the element in the first col of tempMat is 1 or not
%     nElement_5th = find(tempMat(nRowOfKernel,:));  % taking the last row
%     if(length(nElement_5th) == B_2)
%         nElement_1st = find(tempMat(1,:));
%         nElement_2nd = find(tempMat(2,:));
%         nElement_3rd = find(tempMat(3,:));
%         nElement_4th = find(tempMat(4,:));
%
%         if(((isempty(nElement_1st))||(length(nElement_1st) < B_2))&&((isempty(nElement_2nd))||(length(nElement_2nd) < B_2))&&((isempty(nElement_3rd))||(length(nElement_3rd) < B_2))...
%                 &&((isempty(nElement_4th))||(length(nElement_4th) < B_2)))
%
%             Y1 = findTop;
%             conditionFlagOfY1 = 1;
%             break;
...
% conditionFlagOfY2 = 0;
% Y2 = nRw;
% for findBottom = (nRw/2):1:(nRw-(nRowOfKernel-1))
%     tempMat = horiHistMat(findBottom:((findBottom+nRowOfKernel)-1),1:B_2); % generating the kernel
%
%     % checking if all the element in the first col of tempMat is 1 or not
%     nElement_1st = find(tempMat(1,:)); % taking the first row
%     if(length(nElement_1st) == B_2)
%
%         nElement_2nd = find(tempMat(2,:));
%         nElement_3rd = find(tempMat(3,:));
%         nElement_4th = find(tempMat(4,:));
%         nElement_5th = find(tempMat(nRowOfKernel,:));
%         if(((isempty(nElement_5th))||(length(nElement_5th) < B_2))&&((isempty(nElement_2nd))||(length(nElement_2nd) < B_2))&&((isempty(nElement_3rd))||(length(nElement_3rd) < B_2))...
%                 &&((isempty(nElement_4th))||(length(nElement_4th) < B_2)))
%
%             Y2 = findBottom;
%             conditionFlagOfY2 = 1;
...
%%
if((((size(Img1,2)) - endText) > 10))%&&((nRw-bottomEnd)>10))
    %Img1 = Img1(topStart:bottomEnd+10,startText:endText+10); % just to give some relaxation
    Img1 = Img1(:,startText:endText+10);
else
    %Img1 = Img1(topStart:bottomEnd,startText:endText);
    Img1 = Img1(:,startText:endText);
end
% figure,imshow(verticalHistMat),impixelinfo;
